...
{
	int userInput;
	cout << "What is the max capacity of the airplane?: ";
	cin >> userInput;
	m_airport.SetMaxCapacity(userInput);
	...
	while (m_state != FINISHED)
	{
		...
		case WAITING:
			...
			break;
		case LINEUP:
			if (LineUp() == true) { m_state = BOARDING; }
			break;
		case BOARDING:
			if (Board() == true) { m_state = DISEMBARKING; }
			break;
		case DISEMBARKING:
			if (Disembark() == true) { m_state = FINISHED; }
			break;
		}
		m_timeStamp++;
...
/**
    ...
    4. Also add this traveller to the m_ptrTravellers list that the AirTravelSimulator keeps track of.
...
bool AirTravelSimulator::LineUp()
{
	...
	Traveller* randomTravellerPtr = m_pplManager.GetTraveller(randomIndex);
	m_airport.LineUp(randomTravellerPtr);
	m_ptrTravellers.PushBack(randomTravellerPtr);
	randomTravellerPtr->state = LINEUP;
	DisplayMessage(randomTravellerPtr, LINEUP);
...
{
	Traveller* nextTravellerPtr = m_airport.NextInLine();
	m_airplane.Board(nextTravellerPtr);
	nextTravellerPtr->state = BOARDING;
	DisplayMessage(nextTravellerPtr, BOARDING);
...
{
	Traveller* nextOffPlanePtr = m_airplane.Disembark();
	nextOffPlanePtr->state = FINISHED;
	DisplayMessage(nextOffPlanePtr, DISEMBARKING);
